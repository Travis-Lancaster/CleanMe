/**
 * Since there are many menus in the full version, extract sort here for easy maintenance
 *
 * Menu sorting, smaller values appear first, preferably increment by 10 as base
 */

export const home = 1;
export const access = 10;
export const routeNest = 20;

export const outside = 40;

export const exception = 90;
export const system = 100;
export const personalCenter = 110;
export const about = 120;
